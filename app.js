const banks = window.QUIZ_BANKS || [
  {
    id: "assistenti",
    label: "Area Assistenti",
    source: "ASSISTENTI_AUTOFORMAZIONE_PEO.pdf",
...
const els = {
  bank: document.querySelector("#quiz-bank"),
...
function activeBank() {
  return banks.find((bank) => bank.id === state.bankId) || banks[0];
}
...
function populateBankSelect() {
  els.bank.replaceChildren();
  banks.forEach((bank) => {
    const option = document.createElement("option");
    option.value = bank.id;
    option.textContent = bank.label;
    els.bank.append(option);
  });
  els.bank.value = state.bankId;
}

function startSession(keepResults = false) {
  state.order = shuffle(activeQuestions()).map(buildQuestionInstance);
  state.current = 0;
  if (!keepResults) {
...
function updateStats() {
  ...
  els.total.textContent = String(activeQuestions().length);
...
function render() {
  ...
  els.sourceId.textContent = `${activeBank().label} · PDF n. ${formatId(question.id)}`;
